...
data1 <- read.table(file="bigraph_celcomp_rare.txt", sep = "\t",header=T, quote = '"')		# bigraphフォーマット？のデータをロードする
data2 <- read.table(file="vertex_celcomp_rare.txt", sep = "\t", header=T, quote = '"')
...
pdf("image_celcomp_rare.pdf",width=10,height=10)		# 出力を画像ファイル(.pdf)にする。画像サイズは文字のサイズに影響する
