...
data1 <- read.table(file="bigraph_bioproc.txt", sep = "\t",header=T, quote = '"')		# bigraphフォーマット？のデータをロードする
data2 <- read.table(file="vertex_bioproc.txt", sep = "\t", header=T, quote = '"')
...
pdf("image_bioproc.pdf",width=100,height=100)		# 出力を画像ファイル(.pdf)にする。画像サイズは文字のサイズに影響する
plot(net,vertex.size = 2,#E(net)$freq*2,	# ○のサイズ。データ依存にできるはず E(net)$freq*4
		 vertex.color = V(net)$color,	# "white", 		# ○の塗り色
		 vertex.label = V(net)$name,
		 vertex.label.font = 2, 		# ラベル文字をboldモード(2番)にする。3だとイタリックとか
		 vertex.label.cex = 2.0, 		# ラベル文字のサイズ
...
#		 edge.width=10, 				# 線の太さをデータ依存にする。"freq" は元データのヘッダーの記号
#		 edge.color="black",
