...
data1 <- read.table(file="bigraph_celcomp_orphan.txt", sep = "\t",header=T, quote = '"')		# bigraphフォーマット？のデータをロードする
data2 <- read.table(file="vertex_celcomp_orphan.txt", sep = "\t", header=T, quote = '"')
...
pdf("image_celcomp_orphan.pdf",width=30,height=30)		# 出力を画像ファイル(.pdf)にする。画像サイズは文字のサイズに影響する
plot(net,vertex.size = 5,#E(net)$freq*2,	# ○のサイズ。データ依存にできるはず E(net)$freq*4
