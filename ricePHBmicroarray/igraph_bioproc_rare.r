...
data1 <- read.table(file="bigraph_bioproc_rare.txt", sep = "\t",header=T, quote = '"')		# bigraphフォーマット？のデータをロードする
data2 <- read.table(file="vertex_bioproc_rare.txt", sep = "\t", header=T, quote = '"')
...
pdf("image_bioproc_rare.pdf",width=100,height=100)		# 出力を画像ファイル(.pdf)にする。画像サイズは文字のサイズに影響する
