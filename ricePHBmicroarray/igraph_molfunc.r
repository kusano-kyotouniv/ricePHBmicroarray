...
data1 <- read.table(file="bigraph_molfunc.txt", sep = "\t",header=T, quote = '"')		# bigraphフォーマット？のデータをロードする
data2 <- read.table(file="vertex_molfunc.txt", sep = "\t", header=T, quote = '"')

# palf <- colorRampPalette(c("pink", "skyblue")) 
# pal <- palf(7)
...
# pal <- palf(data2$color)

pdf("image_molfunc.pdf",width=300,height=300)		# 出力を画像ファイル(.pdf)にする。画像サイズは文字のサイズに影響する
